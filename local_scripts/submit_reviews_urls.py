#!/usr/bin/env python3
"""Submit one correct review for WaniKani subjects from URLs.

If an item is still in the lesson queue (SRS stage 0), starts the lesson first
so the review can be submitted.

Usage:
    python local_scripts/submit_reviews_urls.py URL [URL ...]
    python local_scripts/submit_reviews_urls.py --file urls.txt
    python local_scripts/submit_reviews_urls.py --dry-run URL [URL ...]
"""
...
def lookup_subject(token: str, api_type: str, slug: str) -> dict:
    types_to_try = [api_type]
    if api_type == "vocabulary":
        types_to_try.append("kana_vocabulary")
    for subject_type in types_to_try:
        lookup_url = (
            f"{SUBJECTS_URL}?"
            + urllib.parse.urlencode({"slugs": slug, "types": subject_type})
        )
...
def submit_correct_review(token: str, subject_id: int, *, dry_run: bool) -> dict | None:
    if dry_run:
        return None
    body = json.dumps(
        {
...
def process_url(token: str, url: str, *, dry_run: bool) -> str:
    ...
    if stage == 0:
        if dry_run:
            return f"dry-run | {label} | would start lesson and submit review (stage 0)"
        start_assignment(token, assignment["id"], dry_run=dry_run)
        assignment = fetch_assignment(token, subject_id)
        if assignment is None:
            return f"error | {label} | assignment missing after start"
        stage = assignment["data"]["srs_stage"]

    available_at = assignment["data"].get("available_at")
    if available_at is None:
        return f"skip | {label} | not yet available for review (available_at is null)"

    if dry_run:
        return f"dry-run | {label} | would submit correct review (SRS stage {stage})"

    try:
        response = submit_correct_review(token, subject_id, dry_run=dry_run)
    except urllib.error.HTTPError as exc:
        body = exc.read().decode("utf-8", errors="replace")
        return f"error | {label} | review failed ({exc.code}): {body}"

    if response and response.get("error"):
        return f"error | {label} | review failed: {response['error']}"

    new_assignment = fetch_assignment(token, subject_id)
    new_stage = new_assignment["data"]["srs_stage"] if new_assignment else "?"
    return f"done | {label} | reviewed (SRS stage {stage} -> {new_stage})"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Submit correct reviews for WaniKani subject URLs.",
    )
